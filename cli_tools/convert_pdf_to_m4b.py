...
def package_book(folder: Path, book_id: str, manifest: dict) -> Path | None:
    ...
    if not audio_metadata:
        logging.warning(
            "Manifest for %s contains no ready chapters; skipping M4B packaging.",
            book_id,
        )
        return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a PDF file into an audiobook packaged as a single .m4b file."
    )
    parser.add_argument("--input", "-i", help="Path to the source PDF file.")
    parser.add_argument(
        "--output-dir",
        "-o",
...
def main() -> int:
    ...
    if not args.input:
        logging.error("No input PDF provided. Use --input /path/to/book.pdf.")
        return 1

    input_path = Path(args.input).expanduser().resolve()
    if not input_path.exists():
        logging.error("Input PDF does not exist: %s", input_path)
        return 1
    if input_path.suffix.lower() != ".pdf":
        logging.warning("Input file does not have .pdf extension: %s", input_path)
    ...
    logging.info(
        "Starting conversion for %s (voice=%s, device=%s)",
        input_path,
        args.voice,
        args.device,
    )
    ...
    # Preserve original PDF alongside outputs for convenience.
    try:
        pdf_copy = output_folder / f"{book_id}.pdf"
        shutil.copy2(input_path, pdf_copy)
        logging.info("Copied source PDF to %s", pdf_copy)
        pdf_copy_name = pdf_copy.name
    except Exception as exc:
        logging.warning("Unable to copy source PDF: %s", exc)
        pdf_copy_name = None
    ...
    logging.warning(
        "Audiobook generation completed without an M4B package. Check logs for details."
    )
    return 2
